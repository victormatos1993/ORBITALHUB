...
export function SupplierForm({ initialData }: SupplierFormProps) {
    ...
    async function onSubmit(data: SupplierFormValues) {
        ...
        try {
            ...
            if (initialData) {
                const result = await updateSupplier(initialData.id, formattedData)
                if (result.error) {
                ...
                const result = await createSupplier(formattedData)
                if (result.error) {
                    toast.error(result.error)
                } else {
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Input placeholder="Nome do fornecedor" {...field} disabled={isSubmitting} />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Input placeholder="email@empresa.com" {...field} disabled={isSubmitting} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
                        ...
                        <div className="flex justify-end space-x-4">
                            ...
                            <Button type="submit" disabled={isSubmitting}>
                                {isSubmitting ? "Salvando..." : (initialData ? "Salvar Alterações" : "Criar Fornecedor")}
                            </Button>
                        </div>
                    </form>
                </Form>
